...
// src/config/azureConfig.js (ESM) — handles both local and App Service using driver-supported auth types
import sql from 'mssql';
import { DefaultAzureCredential } from '@azure/identity';

function log(...args) { console.log(`[${new Date().toISOString()}] [src/config/azureConfig.js]`, ...args); }
function logError(...args) { console.error(`[${new Date().toISOString()}] [src/config/azureConfig.js]`, ...args); }

const credential = new DefaultAzureCredential();


// Accept multiple common key names so local/dev/portal variants work
const SQL_SERVER =
...
if (!SQL_SERVER || typeof SQL_SERVER !== 'string' || !SQL_SERVER.includes('.database.windows.net')) {
  ...
  throw new Error(`Invalid SQL server FQDN; expected like 'albanyregional.database.windows.net', got '${SQL_SERVER || ''}'`);
}
