...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!submitBtn && form) submitBtn = form.querySelector('button.btn.btn-primary');
  ...
  async function checkEmailExists(email) {
    ...
    if (!normalized) { emailTaken = null; return false; }
    ...
    try {
      const url = `/api/volunteers/exists?email=${encodeURIComponent(normalized)}`;
  ...
  function reevaluateEmailGates() {
    ...
    if (emailDeliverable && emailsMatch) {
      ...
      debounceId = setTimeout(() => { checkEmailExists(email); }, 300);
    }
  }
  ...
  if (confirmPasswordInput) {
    confirmPasswordInput.addEventListener('input', maybePreloadDuringPasswordTyping);
  }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!(emailDeliverable && emailsMatch)) {
      e.preventDefault();
      setEmailError('Please validate your email and ensure both entries match.');
      return;
    }
